...
TESTING = True  # Set to False to enable actual CAN and PLC communication
...
class InverterController:
    ...
    def send_message(self, message_id, data):
        if not TESTING:
            ...
            logging.info(f'Mock send CAN message: {message_id} data: {data}')
...
class PLCController:
    ...
    def write_output(self, byte_index, bit_index, value):
        if not TESTING:
            ...
            logging.info(f"Mock write PLC Output {byte_index}.{bit_index} set to {value}")
    ...
    def read_input(self, byte_index, bit_index):
        if not TESTING:
            ...
            logging.info(f"Mock read PLC Input {byte_index}.{bit_index} is False")
            return False

    def disconnect(self):
        if not TESTING and self.client:
            self.client.disconnect()
            logging.info("Disconnected from PLC")
        else:
            logging.info("Mock disconnected from PLC")
